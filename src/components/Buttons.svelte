...
<header class="lg:fixed lg:bottom-0 fixed bottom-0 flex gap-2 dark:bg-black dark:text-white">
	...
	{#if showing === "createSong"}
		<div class="border-1 absolute -top-20 left-0 border-slate-400 p-4">
			<p>Create new song?</p>
			<button
				onclick={() => {
					newSong();
	...
	{#if showing === "showSongs"}
		<div class="border-1 absolute -top-20 left-12 border-slate-400 p-4">
			<a
	...
	{#if showing === "showSaveSuccessful"}
		<p
			use:closeSelf
			class="animate-custom-fade animate-duration-1050 absolute -top-20 left-32 w-max rounded-md bg-green-500 p-2 text-center font-semibold text-white">
