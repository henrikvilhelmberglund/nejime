...
<header class="fixed bottom-0 flex gap-4 dark:text-white">
	...
	{#if showing === "createSong"}
		<div class="border-1 absolute left-0 -top-20 border-slate-400 p-4">
			<p>Create new song?</p>
			<button
				onclick={() => {
					newSong();
	...
	{#if showing === "showSongs"}
		<div class="border-1 absolute left-12 -top-20 border-slate-400 p-4">
			<a
	...
	{#if showing === "showSaveSuccessful"}
		<p
			use:closeSelf
			class="animate-custom-fade animate-duration-1050 absolute left-32 -top-20 w-max rounded-md bg-green-500 p-2 text-center font-semibold text-white">
