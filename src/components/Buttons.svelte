...
<header class="relative flex gap-4">
	<button
		onclick={() => {
			toggle("createSong");
		}}
	...
	{#if showing === "createSong"}
		<div class="border-1 absolute left-0 top-16 border-slate-400 p-4">
			<p>Create new song?</p>
			<button
				onclick={() => {
					newSong();
	...
	{#if showing === "showSongs"}
		<div class="border-1 absolute left-12 top-16 border-slate-400 p-4">
			<a
